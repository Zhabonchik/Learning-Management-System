...
public record CreateCourseSettingsDto(
        @NotNull(message = "Start date must not be null") @JsonFormat(pattern = DateFormatUtils.DATE_TIME_FORMAT)
        @FutureOrPresent(message = "Start date must not be in past") LocalDateTime startDate,
        @NotNull(message = "End date must not be null") @JsonFormat(pattern = DateFormatUtils.DATE_TIME_FORMAT)
